      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="glass p-6 space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Build a Real-Time Dashboard" className="bg-secondary/50 border-border" />
          </div>

          <div className="space-y-2">
            <Label className="text-foreground">Description *</Label>
            <Textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Describe the challenge in detail..." rows={5} className="bg-secondary/50 border-border" />
          </div>
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              <Input type="date" value={deadline} onChange={(e) => setDeadline(e.target.value)} className="bg-secondary/50 border-border" />
            </div>
            <div className="space-y-2">
              <Label className="text-foreground">Max Participants</Label>
              <Input type="number" value={maxParticipants} onChange={(e) => setMaxParticipants(e.target.value)} className="bg-secondary/50 border-border" />
            </div>
            <div className="space-y-2">
              <Label className="text-foreground">Prize</Label>
              <Input value={prize} onChange={(e) => setPrize(e.target.value)} placeholder="$2,500" className="bg-secondary/50 border-border" />
            </div>
          </div>

          <div className="space-y-2">
            <Label className="text-foreground">XP Reward</Label>
            <Input type="number" value={xpReward} onChange={(e) => setXpReward(e.target.value)} className="bg-secondary/50 border-border max-w-[200px]" />
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="font-heading font-semibold text-foreground">Requirements</h2>
            <Button type="button" variant="ghost" size="sm" onClick={addRequirement} className="text-primary hover:text-primary/80">
          ...
          {requirements.map((req, i) => (
            <div key={i} className="flex gap-2">
              ...
              {requirements.length > 1 && (
                <Button type="button" variant="ghost" size="icon" onClick={() => removeRequirement(i)} className="text-muted-foreground hover:text-destructive flex-shrink-0">
                  <Trash2 size={14} />
                </Button>
              )}
            </div>
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass p-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-heading font-semibold text-foreground">Scoring Criteria</h2>
          ...
          {criteria.map((c, i) => (
            <div key={i} className="glass-subtle p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                {criteria.length > 1 && (
                  <Button type="button" variant="ghost" size="icon" onClick={() => removeCriterion(i)} className="text-muted-foreground hover:text-destructive h-6 w-6">
                    <Trash2 size={12} />
                  </Button>
                )}
              </div>
              <div className="grid sm:grid-cols-[1fr,80px] gap-3">
                <Input value={c.name} onChange={(e) => updateCriterion(i, "name", e.target.value)} placeholder="e.g. Code Quality" className="bg-secondary/50 border-border" />
                <Input type="number" value={c.weight} onChange={(e) => updateCriterion(i, "weight", e.target.value)} placeholder="%" className="bg-secondary/50 border-border" />
              </div>
              <Input value={c.description} onChange={(e) => updateCriterion(i, "description", e.target.value)} placeholder="Describe what you're scoring..." className="bg-secondary/50 border-border" />
            </div>
          ))}
        </motion.div>
